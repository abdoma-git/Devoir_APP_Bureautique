...
cursor.execute("CREATE TABLE IF NOT EXISTS "
               "matchs_nouveau(id INTEGER PRIMARY KEY AUTOINCREMENT, "
               "equipe1 VARCHAR(20), "
               "equipe2 VARCHAR(20), "
...
def inserer_user(*args):
    cursor.execute(" INSERT INTO users(nom, prenom, tel, mail, mdp) "
                   "VALUES ( ?, ?, ?, ?, ?) ", args)
    connection.commit()
def liste_users():
...
def inserer_equipe(*args):
    cursor.execute(" INSERT INTO equipes(nom, pays, joueur1, joueur2, "
...
def inserer_joueur(*args):
    cursor.execute(" INSERT INTO joueurs(nom, prenom, numero, pays) "
                   "VALUES ( ?, ?, ?, ?) ", args)
    connection.commit()
...
def inserer_match(*args):
    cursor.execute(" INSERT INTO matchs_nouveau(equipe1, equipe2, "
